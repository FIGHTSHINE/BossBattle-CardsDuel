"""Dynamic player health bar widget."""
...
class PlayerHealthBar(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # HP label (changed for player)
        self.hp_label = Label(
            text=f"🛡️ PLAYER: {self.current_hp}/{self.max_hp}",
            font_size='20sp',
            size_hint_y=0.3,
            bold=True,
            color=(1, 1, 1, 1),
    ...
    def on_hp_change(self, instance, value):
        """Handle HP change."""
        # Update text (changed for player)
        self.hp_label.text = f"🛡️ PLAYER: {self.current_hp}/{self.max_hp}"
